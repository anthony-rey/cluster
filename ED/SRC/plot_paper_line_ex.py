...
dataFolder = "../DATA/LINE_EX/"
graphsFolder = "../GRAPHS/LINE_EX/"
...
for t in range(len(theta)):
	for i in range(9):
		...
		for n in range(len(Ns)):
			p = ax[i].plot(lam, Ns[n]*diffsNs[n, :, t, i], alpha=1, linestyle=":", markeredgewidth=1.4, markersize=6, linewidth=0.6, zorder=100, label=fr"$2N = {Ns[n]}$")
			[p[s].set_marker(mpl.markers.MarkerStyle(p[s].get_marker(), capstyle="round")) for s in range(len(p))]

		ax[i].axvline(0.5, color="k", linewidth=1, linestyle="--")
	...
	if args.save:
		saveGraph(fig, os.path.join(graphsFolder, f"pbc={PBCs}_arpack={useArpack}_isoTheta", f"theta={np.round(theta[t], 3)}"), f"gaps")