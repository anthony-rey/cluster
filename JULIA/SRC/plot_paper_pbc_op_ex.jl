...
if PBCs
	graphFolder = "../GRAPHS/PBC/OP_EX/"
	dataFolder = "../DATA/PBC/OP_EX/"
else
	graphFolder = "../GRAPHS/OBC/OP_EX/"
	dataFolder = "../DATA/OBC/OP_EX/"
end
...
for n=1:length(Ns_)
	...
	magsXSPT = numpy.zeros((length(chisN), length(lam), length(theta), 6))
	...
	for x=1:length(chisN)
		...
		for i=1:length(dataFilenames)
			...
			if plot_mags_chi
				...
				magsXSPT[x, vals[i]["lam"].==lam, vals[i]["theta"].==theta, 1] .= numpy.mean(mX)
				magsXSPT[x, vals[i]["lam"].==lam, vals[i]["theta"].==theta, 2] .= numpy.mean([mX[l]*(-1)^l for l=1:length(mX)])
				magsXSPT[x, vals[i]["lam"].==lam, vals[i]["theta"].==theta, 3] .= numpy.mean(mY)
				magsXSPT[x, vals[i]["lam"].==lam, vals[i]["theta"].==theta, 4] .= numpy.mean([mY[l]*(-1)^l for l=1:length(mY)])
				magsXSPT[x, vals[i]["lam"].==lam, vals[i]["theta"].==theta, 5] .= numpy.mean(mZ)
				magsXSPT[x, vals[i]["lam"].==lam, vals[i]["theta"].==theta, 6] .= numpy.mean([mZ[l]*(-1)^l for l=1:length(mZ)])
			end
		end
	end
	...
	if plot_mags_chi
		...
		for l=1:length(lam)
			for t=1:length(theta)
				...
				p, = ax.plot(chisN, numpy.abs(magsX[:, l, t, 2])+numpy.abs(magsXSPT[:, l, t, 4]), linewidth=1, markeredgewidth=2, zorder=105, label=latexstring(L"$2N=%$(Ns_[n])$"))
...
ax.set_ylabel(latexstring(L"$|m^{x}_{\mathrm{sta}}|+|m^{y}_{\mathrm{sta,SPT}}|$"))
