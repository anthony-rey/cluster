...
if PBCs
	graphFolder = "../GRAPHS/PBC/OP_DX/"
	dataFolder = "../DATA/PBC/OP_DX/"
else
	graphFolder = "../GRAPHS/OBC/OP_DX/"
	dataFolder = "../DATA/OBC/OP_DX/"
end
	...
	for x=1:length(chisN)
		...
		for i=1:length(dataFilenames)
			...
			if plot_mags_chi
				...
				mX = data["mags"][1, 1, 10:end-10]
				mY = data["mags"][1, 2, 10:end-10]
				mZ = data["mags"][1, 3, 10:end-10]
...
ax.minorticks_on()
ax.legend(loc="center", ncol=1, labelspacing=0.8, columnspacing=0.8, handletextpad=0.6, handlelength=1.8).set_zorder(110)

ax.set_xlabel(latexstring(L"$\chi$"))
ax.set_ylabel(latexstring(L"$|m^{x}_{\mathrm{sta}}|+|m^{z}_{\mathrm{uni}}|$"))
