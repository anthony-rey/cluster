...
if plot_scaling_en
	...
	for l=1:length(lam)
		for t=1:length(theta)
			...
			linestyles = ["-", ":", "--", ":", "-"]
			cmap = matplotlib.colors.LinearSegmentedColormap.from_list("...", colors=[colors[1],colors[3],colors[7],colors[6],colors[4]], N=256)
			PyPlot.rc("axes", prop_cycle=cycler.cycler(marker=[".", "|", "_", "|", "."], markersize=2 .*[10, 10, 10, 10, 10], markeredgewidth=2 .*[1, 1.5, 1.5, 1.5, 1], color=cmap(numpy.linspace(0, 1, nE-1))))
			...
			if lam[l] in [0., 0.01, 0.05, 0.1]
				...
				for k=1:nE-1
					...
					o, = ax[s].plot(1 ./Ns, py"f"(Ns, popt...), linestyle=linestyles[k], marker="none", color=p.get_color(), linewidth=2, zorder=90+k, markersize=0, markeredgewidth=0, label=latexstring(L"$v^{\,}_{%$(k),%$(k-1)}(%$(lam[l])) = %$(round(popt[1]; digits=2)) \pm %$(round(perr[1]; digits=2))$"))
					push!(os, o)
...
for t=1:length(theta)
	...
	o2, = ax[2].plot(x, py"g"(x, popt...), markersize=0, linewidth=3, color=colors[1], zorder=100, label=latexstring(L"$v^{\,}_{3,2} - %$(round(popt[3]; digits=3)) \sim \lambda^{%$(round(popt[2]; digits=2)) \pm %$(round(perr[2]; digits=2))}$"))
	...
	p3, = ax[3].plot(lam, slopes[:, t, 4], marker="x", markersize=10, color=colors[7], linewidth=0, markeredgewidth=3, zorder=100)
	p3.set_marker(matplotlib.markers.MarkerStyle(p3.get_marker(), capstyle="round"))
	o3, = ax[3].plot(x, py"g"(x, popt...), markersize=0, linewidth=3, color=colors[1], zorder=100, label=latexstring(L"$v^{\,}_{4,3} - %$(round(popt[3]; digits=3)) \sim \lambda^{%$(round(popt[2]; digits=3)) \pm %$(round(perr[2]; digits=3))}$"))

	println(slopes[:, t, 1], slopes[:, t, 3], slopes[:, t, 4])

	ax[1].set_title(latexstring(L"$E^{\,}_{1} - E^{\,}_{0}$"), pad=10)
	ax[2].set_title(latexstring(L"$E^{\,}_{3} - E^{\,}_{2}$"), pad=10)
	ax[3].set_title(latexstring(L"$E^{\,}_{4} - E^{\,}_{3}$"), pad=10)

	ax[1].set_xlabel(latexstring(L"$\lambda$"))
	ax[2].set_xlabel(latexstring(L"$\lambda$"))
	ax[3].set_xlabel(latexstring(L"$\lambda$"))

	ax[1].set_ylabel(latexstring(L"$v^{\,}_{1,0}$"))
	ax[2].set_ylabel(latexstring(L"$v^{\,}_{3,2}$"))
	ax[3].set_ylabel(latexstring(L"$v^{\,}_{4,3}$"))
	...
	ax[3].legend(fontsize=14, handlelength=1.8, handletextpad=0.6, labelspacing=0.8, loc="lower left").set_zorder(200)
