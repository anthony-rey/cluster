...
# in case we want to revert back to original colors
# colors = ["salmon", "mediumredviolet", "k", "", "steelblue", ""]
...
for folder in folders
	...
	for i=1:length(dataFilenames)
		...
		# 	axes[2].plot(r_, py"oz"(collect(r_), poptoz...), ls="-", lw=1, color="k", zorder=101, label=latexstring(L"$C^{z}$ fit$^{\,}_{\mathrm{q}}$"))
		# catch
		# 	poptoz = 0
		# 	perroz = 0
		# 	poz = 0
		...
		# 	println(poptfy, perrfy)

		#  ax.plot(sites, py"f"(collect(sites), poptfy...), ls="--", lw=1, color=colors[4], zorder=100, label=latexstring(L"$|C^{\,}_{y}|$ fit$^{\,}_{\mathrm{p}}$"))
		# catch
		# 	poptfy = 0
		# 	perrfy = 0
		# 	pfy = 0
		...
		#  ax.plot(sites, py"g"(collect(sites), poptgy...), ls=":", lw=1, color=colors[4], zorder=101, label=latexstring(L"$|C^{\,}_{y}|$ fit$^{\,}_{\mathrm{e}}$"))
		# catch
		# 	poptgy = 0
		# 	perrgy = 0
		# 	pgy = 0
		...
		# axes[2].set_ylim(-0.1, 0.21)
	 	...
	 	ax.set_xticks([0, 10, 20, 30, 40, 50, 64])
		# axes[2].set_xticks([0, 10, 20, 30, 40, 50, 64])
		...
		sort = numpy.argsort(ftfreq).+1
		axins1.plot(ftfreq[sort], numpy.abs(ft[sort]), marker=".", linestyle=":", markersize=3, linewidth=0.5, color=colors[4], zorder=110, label=latexstring(L"FT$^{y}$"))
