...
if PBCs
	graphFolder = "../graphs/pbc/lineC•EU(1)SPT/"
	dataFolder = "../data/pbc/lineC•EU(1)SPT/"
else
	graphFolder = "../graphs/obc/lineC•EU(1)SPT/"
	dataFolder = "../data/obc/lineC•EU(1)SPT/"
end
...
for x=1:length(chis_)
	...
	cmap = matplotlib.colors.LinearSegmentedColormap.from_list("...", colors=[colors[6],colors[4],colors[7],colors[3],colors[1]], N=256)
	PyPlot.rc("axes", prop_cycle=cycler.cycler(linestyle=["--", "--", ":", "-", "-"], marker=[".", "x", "+", ".", "x"], markersize=[10, 7.5, 10, 10, 7.5], markeredgewidth=[1.5, 1.5, 1.5, 1.5, 1.5], color=cmap(numpy.linspace(0, 1, 5))))

	fig_combined, ax_combined = plt.subplots(figsize=(4, 4))
	...
	for n=1:length(NsN)
		...
		for i=1:length(dataFilenames)
			...
			if plot_entropies | plot_lines_c | plot_lines_c_combined
				...
				label=latexstring(L"$S = \frac{(%$(round(popt[1]; digits=5)) \pm %$(round(perr[1]; digits=5)))}{3} \ln\left[\frac{N}{\pi}\sin\frac{\pi \ell}{N} \right] + \mathrm{ const}$")
		...
		if plot_lines_c
			...
			for l=1:length(lam)
				...
				ax.errorbar(theta, cs[l, :, 1], cs[l, :, 2], ecolor=colors[5], elinewidth=1, color=colors[2], marker=".", linewidth=0.5, markersize=5, zorder=105)
		...
		if plot_lines_c_combined
			...
			for l=1:length(lam)
				...
				p, = ax_combined.plot(theta, cs[l, :, 1], linewidth=1, markeredgewidth=2, zorder=105, label=latexstring(L"$2N = %$(Ns[n])$"))
	...
	ax_combined.set_xlabel(latexstring(L"$\theta$"))
	ax_combined.set_ylabel(latexstring(L"$\mathsf{c}$"))

	ax_combined.axvline(pi/4, color="k", linewidth=1, linestyle=":")
