...
if PBCs
	graphFolder = "../GRAPHS/PBC/LINE_C_E+DU(1)_SPT/"
	dataFolder = "../DATA/PBC/LINE_C_E+DU(1)_SPT/"
else
	graphFolder = "../GRAPHS/OBC/LINE_C_E+DU(1)_SPT/"
	dataFolder = "../DATA/OBC/LINE_C_E+DU(1)_SPT/"
end
...
for x=1:length(chis_)
	...
	PyPlot.rc("axes", prop_cycle=cycler.cycler(linestyle=["--", "--", "-", "-"], marker=[".", "x", ".", "x"], markersize=[10, 7.5, 10, 7.5], markeredgewidth=[1.5, 1.5, 1.5, 1.5], color=cmap(numpy.linspace(0, 1, 4))))
	...
	for n=1:length(NsN)
		...
		for i=1:length(dataFilenames)
			...
			if plot_entropies | plot_lines_c | plot_lines_c_combined
				...
				label=latexstring(L"$S = \frac{(%$(round(popt[1]; digits=4)) \pm 2\cdot 10^{-4})}{3} \ln\left[\frac{2N}{\pi}\sin\frac{\pi l}{2N} \right] + \mathrm{ const}$")

				ax.scatter(sites, S, marker=matplotlib.markers.MarkerStyle("x", capstyle="round"), s=50, linewidths=2, color=colors[1], zorder=100)
		...
		if plot_lines_c
			...
			for t=1:length(theta)
				...
				p, = ax_line.plot(lam, cs[:, t, 1], color=colors[1], marker=".", linewidth=1, markersize=10, markeredgewidth=2, linestyle="-", zorder=105)
				p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))		

				ax_line.minorticks_on()
				# ax_line.grid(which="minor", linewidth=0.2)
				# ax_line.grid(which="major", linewidth=0.6)

				ax_line.set_xlabel(latexstring(L"$\lambda$"))
				ax_line.set_ylabel(latexstring(L"$\mathsf{c}$"))

				plt.draw()

				if savePlot
					py"createFolderIfMissing"(graphFolder * folder)
					fig_line.savefig(graphFolder * folder * "win=$(win)_theta=$(theta[t]).pdf")
					ax_line.cla()
				end
			end
		end
	...
	ax_combined.legend(loc="upper right", ncol=1, labelspacing=0.8, columnspacing=0.8, handletextpad=0.6, handlelength=1.8).set_zorder(110)
