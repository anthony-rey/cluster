...
for folder in folders
		...
		# ------------------------------
			...
			if plot_correlations | plot_lines_correlations
				...
				for j=1:k
					...
					plt.plot(range(1, r), corrZ_conn, marker=6, color=colors[3], ls="-", lw=0.5, markersize=4, label=latexstring(L"$C^{{\,}}_{{z}}$"), zorder=100)
					...
					try
						r_ = range(1, r)[start_conn:end+1-end_]
						corrZ_ = corrZ_conn[r_]

						poptozconn, pcov = scipy.optimize.curve_fit(py"oz_conn", collect(r_), corrZ_, [0.05, 0.31, -1, 1.1, 0])
						...
						corrs_alg[vals[i]["lam"].==lam, vals[i]["theta"].==theta, 1] .= poptozconn[4]
						corrs_alg[vals[i]["lam"].==lam, vals[i]["theta"].==theta, 2] .= perrozconn[4]

						plt.plot(sites, py"oz_conn"(collect(sites), poptozconn...), markersize=0, ls="--", lw=1, color=colors[2], zorder=102, label=latexstring(L"$|C^{\,}_{z,c}| \sim \ell^{- %$(round(poptozconn[4]; digits=3)) \pm %$(round(perrozconn[4]; digits=3))}$"))
					catch
						poptozconn = 0
						perrozconn = 0
						pozconn = 0
					...
					if savePlot & plot_correlations
						py"createFolderIfMissing"(graphFolder * folder * "CORRELATIONS_Z/")
						fig.savefig(graphFolder * folder * "CORRELATIONS_Z/" * chop(dataFilenames[i], tail=5) * ".pdf")
					end
					ax.cla()
				end
			end
	...
	popt, pcov = scipy.optimize.curve_fit(py"g", corrs_alg[end-40:end, 1, 1], numpy.abs(mags[end-40:end, 1, 1]), [1, 1])
	perr = numpy.sqrt(numpy.diag(pcov))

	println(popt, perr)

	r2_ = py"r2"(numpy.abs(mags[end-40:end, 1, 1]), py"g"(corrs_alg[end-40:end, 1, 1], popt...))
	# r2_ = py"r2"(corrs_alg[end-40:end, 1, 1], mags[end-40:end, 1, 1])
	println(r2_)

	# ax2.plot(corrs_alg[end-40:end, 1, 1], py"g"(corrs_alg[end-40:end, 1, 1], popt...), linewidth=0.7, marker=".", color=colors[1], markersize=6, zorder=300, label=latexstring(L"$;$"))

	ax1.plot(lam[end-40:end], corrs_alg[end-40:end, 1, 1], linewidth=0.7, marker=".", color=colors[1], markersize=6, zorder=100, label=latexstring(L"$\eta^{z,\mathrm{c}}$"))
	...
	ax2.set_xlabel(latexstring(L"$\eta^{z,\mathrm{c}}$"))
	...
	if savePlot
		...
		fig1.savefig(graphFolder * folder * "sm_z_together_r=$(r_spins)_theta=$(theta[1])" * ".pdf")
		fig1.clf()
		fig2.savefig(graphFolder * folder * "sm_z_against_r=$(r_spins)_theta=$(theta[1])" * ".pdf")
		fig2.clf()
	...
	if plot_lines_correlations
		...
		for t=1:length(theta)
			...
			ax_line.set_ylabel(latexstring(L"$\eta^{z,\mathrm{c}}$"))
			...
			if savePlot
				py"createFolderIfMissing"(graphFolder * folder)
				fig_line.savefig(graphFolder * folder * "line_eta_z" * ".pdf")
				fig_line.clf()
			end
