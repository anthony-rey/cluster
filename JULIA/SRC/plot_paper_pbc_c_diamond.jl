...
if PBCs
	graphFolder = "../GRAPHS/PBC/C_DIAMOND/"
	dataFolder = "../DATA/PBC/C_DIAMOND/"
else
	graphFolder = "../GRAPHS/OBC/C_DIAMOND/"
	dataFolder = "../DATA/OBC/C_DIAMOND/"
end
...
PyPlot.rc("axes", prop_cycle=cycler.cycler(marker=["x", ".", "x", "."], markersize=[5, 10, 5, 10], markeredgewidth=[1.5, 1.5, 1.5, 1.5], color=cmap(numpy.linspace(0, 1, length(Ns)))))
...
for folder in folders
	...
	for i=1:length(dataFilenames)
		...
		if plot_entropies_combined
			...
			x = 1/3 * numpy.log(N/numpy.pi .* numpy.sin(numpy.pi .*sites ./N))
			...
			plt.xlabel(latexstring(L"$\frac{1}{3} \ln \left[ \frac{2N}{\pi} \sin\frac{\pi l}{2N} \right]$"))
...
p, = axin.plot(Ns, cs .-1, marker="x", markersize=5, markeredgewidth=1.5, linewidth=0.5, color=colors[7], zorder=110)
p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))

axin.set_xticks([0, 50, 100, 150])

axin.set_ylim(0.0001, 0.18)

axin.set_xlabel(L"$2N$")
axin.set_ylabel(L"$\mathsf{c}-1$")
