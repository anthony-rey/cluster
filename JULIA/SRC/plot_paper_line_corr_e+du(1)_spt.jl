...
if PBCs
	graphFolder = "../GRAPHS/PBC/LINE_CORR_E+DU(1)_SPT/"
	dataFolder = "../DATA/PBC/LINE_CORR_E+DU(1)_SPT/"
else
	graphFolder = "../GRAPHS/OBC/LINE_CORR_E+DU(1)_SPT/"
	dataFolder = "../DATA/OBC/LINE_CORR_E+DU(1)_SPT/"
end

computeFolder = "ANALYZED/"
folders = py"folderNames"(dataFolder)

plot_correlations_spt = true
plot_lines_correlations_spt = true
...
for folder in folders
		...
		# ------------------------------
			...
			if plot_correlations_spt
				...
				for j=1:k
					...
					p, = plt.plot(range(1, r), corrX, marker="3", color=colors[1], ls="-", lw=0.5, markersize=7, markeredgewidth=1.5, label=latexstring(L"$C^{\mathrm{SPT},x}$"), zorder=100)
					p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
					p, = plt.plot(range(1, r), corrY, marker="4", color=colors[4], ls="-", lw=0.5, markersize=7, markeredgewidth=1.5, label=latexstring(L"$C^{\mathrm{SPT},y}$"), zorder=101)
					p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
					p, = plt.plot(range(1, r), corrZ, marker="1", color=colors[7], ls="-", lw=0.5, markersize=7, markeredgewidth=1.5, label=latexstring(L"$C^{\mathrm{SPT},z}$"), zorder=102)
					p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
					...
					try
						...
						plt.plot(sites, py"f"(collect(sites), poptfx...), ls="--", lw=1, color=colors[1], zorder=103, label=latexstring(L"$|C^{\mathrm{SPT},x}| \sim s^{- %$(round(poptfx[2]; digits=3)) \pm %$(round(perrfx[2]; digits=3))}, \ p = %$(Int(round(pfx[1]; digits=2)))$"))
					catch
						poptfx = 0
						perrfx = 0
						pfx = 0
					...
					try
						...
						plt.plot(sites, py"g"(collect(sites), poptgx...), ls=":", lw=1, color=colors[1], zorder=103, label=latexstring(L"$|C^{\mathrm{SPT},x}| \sim e^{- s/(%$(round(poptgx[2]; digits=1)) \pm %$(round(perrgx[2]; digits=1)))}, \ p = %$(round(pgx[1]; digits=2))$"))
					catch
						poptgx = 0
						perrgx = 0
						pgx = 0
	...
	if plot_lines_correlations_spt

		fig_line, ax_line = plt.subplots(figsize=(3.4, 3.15))
		...
		for t=1:length(theta)
			...
			ax_line.set_ylabel(latexstring(L"$\eta^{\,}_{\mathrm{SPT}}$"))
