...
for folder in folders
	...
	for i=1:length(dataFilenames)
		...
		# p, = axes[1].plot(range(1, r), corrX, marker="3", color=colors[1], ls="-", lw=0.5, markersize=7, markeredgewidth=1.5, label=latexstring(L"$C^{x}$"), zorder=100)
		# p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
		# p, = axes[1].plot(range(1, r), corrY, marker="4", color=colors[4], ls="-", lw=0.5, markersize=7, markeredgewidth=1.5, label=latexstring(L"$C^{y}$"), zorder=101)
		# p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
		p, = ax.plot(range(1, r), corrZ, marker="1", color=colors[7], ls="-", lw=0.5, markersize=7, markeredgewidth=1.5, label=latexstring(L"$C^{z}$"), zorder=100)
		p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
		...
		try
			...
			ax.plot(r_, py"oz"(collect(r_), poptoz...), ls="-", lw=1, color="0", alpha=1, zorder=101, label=latexstring(L"$C^{z}$ fit$^{\,}_{\mathrm{q}}$"))
		catch
			poptoz = 0
			perroz = 0
			poz = 0
		...
		# 	axes[1].plot(sites, py"f"(collect(sites), poptfx...), ls="--", lw=1, color=colors[1], zorder=100, label=latexstring(L"$|C^{x}|$ fit$^{\,}_{\mathrm{p}}$"))
		# catch
		# 	poptfx = 0
		# 	perrfx = 0
		# 	pfx = 0
		...
		# 	axes[1].plot(sites, py"g"(collect(sites), poptgx...), ls=":", lw=1, color=colors[1], zorder=100, label=latexstring(L"$|C^{x}|$ fit$^{\,}_{\mathrm{e}}$"))
		# catch
		# 	poptgx = 0
		# 	perrgx = 0
		# 	pgx = 0
		...
		# 	axes[1].plot(sites, py"f"(collect(sites), poptfy...), ls="--", lw=1, color=colors[4], zorder=101, label=latexstring(L"$|C^{y}|$ fit$^{\,}_{\mathrm{p}}$"))
		# catch
		# 	poptfy = 0
		# 	perrfy = 0
		# 	pfy = 0
		...
		# 	axes[1].plot(sites, py"g"(collect(sites), poptgy...), ls=":", lw=1, color=colors[4], zorder=101, label=latexstring(L"$|C^{y}|$ fit$^{\,}_{\mathrm{e}}$"))
		# catch
		# 	poptgy = 0
		# 	perrgy = 0
		# 	pgy = 0
		...
		if r==72
			# axes[1].set_xticks([0, 10, 20, 30, 40, 50, 60, 72])
			ax.set_xticks([0, 10, 20, 30, 40, 50, 60, 72])
		end

		plt.xlabel(latexstring(L"$s$"))

		# axes[1].legend(loc="upper left", ncol=2, bbox_to_anchor=(0.06, 0.97), borderaxespad=0, labelspacing=0.4, columnspacing=0.8, handletextpad=0.6, handlelength=1.8).set_zorder(110)
		ax.legend(loc="lower left", ncol=1, bbox_to_anchor=(0.06, 0.03), borderaxespad=0, labelspacing=0.4, columnspacing=0.8, handletextpad=0.6, handlelength=1.8).set_zorder(110)

		# if r==72
		# 	axes[1].annotate("(c)", xy=(0.945, 0.08), xycoords="axes fraction", fontsize=16)
		# 	ax.annotate("(d)", xy=(0.945, 0.08), xycoords="axes fraction", fontsize=16)
		# elseif r==80
		# 	axes[1].annotate("(a)", xy=(0.945, 0.08), xycoords="axes fraction", fontsize=16)
		# 	ax.annotate("(b)", xy=(0.945, 0.08), xycoords="axes fraction", fontsize=16)
		# end

		# axins1 = axes[1].inset_axes([0.58, 0.5, 0.4, 0.45])
		# axins1.set_zorder(102)
		axins2 = ax.inset_axes([0.5, 0.17, 0.4, 0.45])
