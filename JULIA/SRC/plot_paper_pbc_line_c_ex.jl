...
if PBCs
	graphFolder = "../GRAPHS/PBC/LINE_C_EX/"
	dataFolder = "../DATA/PBC/LINE_C_EX/"
else
	graphFolder = "../GRAPHS/OBC/LINE_C_EX/"
	dataFolder = "../DATA/OBC/LINE_C_EX/"
end
...
for n=1:length(Ns_)
	...
	cmap = matplotlib.colors.LinearSegmentedColormap.from_list("...", colors=[colors[1],colors[4],colors[3],colors[6]], N=256)
	PyPlot.rc("axes", prop_cycle=cycler.cycler(linestyle=["--", "--", "-", "-"], marker=["x", "x", ".", "."], markersize=[7.5, 7.5, 10, 10], markeredgewidth=[1.5, 1.5, 1.5, 1.5], color=cmap(numpy.linspace(0, 1, 4))))

	fig_combined, ax_combined = plt.subplots(figsize=(3.5, 4))
	...
	for x=1:length(chisN)
		...
		if plot_lines_c_combined
			...
			for t=1:length(theta)
				...
				p, = ax_combined.plot(lam, cs[:, t, 1], linewidth=1, markeredgewidth=2, zorder=105, label=latexstring(L"$\chi = %$(chisN[x])$"))
	...
	ax_combined.axvline(0.5, color="k", linewidth=1, linestyle="--")

	ax_combined.minorticks_on()
	# ax_combined.grid(which="minor", linewidth=0.2, alpha=0.33)
	# ax_combined.grid(which="major", linewidth=0.6)
	ax_combined.legend(loc="lower center", bbox_to_anchor=(0.5, 1.01), ncol=2, labelspacing=0.8, columnspacing=0.8, handletextpad=0.6, handlelength=1.8).set_zorder(110)

	# ax_combined.set_xlim(0.484, 0.512)
	...
	if savePlot
		...
		fig_combined.savefig(graphFolder * "win=$(win)_N=$(Ns_[n])" * ".pdf")
		fig_combined.clf()
	end
