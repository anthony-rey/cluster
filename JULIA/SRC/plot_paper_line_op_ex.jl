...
if PBCs
	graphFolder = "../GRAPHS/PBC/LINE_OP_EX/"
	dataFolder = "../DATA/PBC/LINE_OP_EX/"
else
	graphFolder = "../GRAPHS/OBC/LINE_OP_EX/"
	dataFolder = "../DATA/OBC/LINE_OP_EX/"
end
...
for folder in folders
	...
	# if N==64
	# 	println(lam[[80:90; 100:110]])
	# 	r_ = lam[[80:90; 100:110]]

	# 	poptf, pcov = scipy.optimize.curve_fit(py"f", collect(r_), numpy.abs(mags[1, [80:90; 100:110], 1, 2]), [1, 0.58, 0.15, 0])
	...
	# 	poptg, pcov = scipy.optimize.curve_fit(py"g", collect(r_), numpy.log(numpy.abs(mags[1, 70:85, 1, 2])), [0.5, 0.51, 1])
	# 	# poptg, pcov = scipy.optimize.curve_fit(py"g", collect(r_), numpy.log(numpy.abs(mags[1, 1, 80:95, 2])), [0.5, 1])
	# 	perrg = numpy.sqrt(numpy.diag(pcov))
	# 	# chi2 = scipy.stats.chisquare(numpy.abs(mags[1:end, t, 1]), py"f"(collect(r_), poptg...), length(r_)-5)
	# 	# poz = chi2[2]
	# 	println(poptg, perrg)

	# 	r2_ = py"r2"(numpy.log(numpy.abs(mags[1, 70:85, 1, 2])), py"g"(collect(r_), poptg...))
	# 	# println(r2_)
	...
	# 	p, = axins.plot(numpy.log(numpy.abs(lam[30:93].-poptg[2])), numpy.log(numpy.abs(mags[1, 30:93, 1, 2])), linewidth=0.1, marker="x", color=colors[4], markersize=6, markeredgewidth=1.5, zorder=100)	
	# 	# # p, = axins.plot(numpy.log(numpy.abs(lam[10:69].-0.65)), numpy.log(numpy.abs(mags[1, 1, 10:69, 2])), linewidth=0.1, marker="x", color=colors[4], markersize=6, markeredgewidth=1.5, zorder=100)	
	# 	# p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
	# end

	# if N==32
	# 	println(lam[[85:94; 110:120]])
	# 	r_ = lam[[85:94; 110:120]]

	# 	poptf, pcov = scipy.optimize.curve_fit(py"f", collect(r_), numpy.abs(mags[1, [85:94; 110:120], 1, 2]), [1, 0.58, 0.15, 0])
	# 	perrf = numpy.sqrt(numpy.diag(pcov))
	# 	# chi2 = scipy.stats.chisquare(numpy.abs(mags[1:end, t, 1]), py"f"(collect(r_), poptf...), length(r_)-5)
	# 	# poz = chi2[2]
	# 	println(poptf, perrf)
	...
	# 	poptg, pcov = scipy.optimize.curve_fit(py"g", collect(r_), numpy.log(numpy.abs(mags[1, 80:94, 1, 2])), [0.5, 0.51, 1])
	# 	# poptg, pcov = scipy.optimize.curve_fit(py"g", collect(r_), numpy.log(numpy.abs(mags[1, 1, 80:95, 2])), [0.5, 1])
	# 	perrg = numpy.sqrt(numpy.diag(pcov))
	# 	# chi2 = scipy.stats.chisquare(numpy.abs(mags[1:end, t, 1]), py"f"(collect(r_), poptg...), length(r_)-5)
	# 	# poz = chi2[2]
	...
	# 	r2_ = py"r2"(numpy.log(numpy.abs(mags[1, 80:94, 1, 2])), py"g"(collect(r_), poptg...))
	# 	# println(r2_)

	# 	axins.plot(numpy.log(numpy.abs(r_.-poptg[2])), py"g"(collect(r_), poptg...), ls="-", lw=2, color="k", zorder=101, label=latexstring(L"$\beta^{\,}_{\mathrm{tri}}\,\ln |\lambda-\lambda^{\,}_{2N,\mathrm{tri}}|+\mathrm{const}$"))
	# 	# # axins.plot(numpy.log(numpy.abs(r_)), py"g"(collect(r_), poptg...), ls="-", lw=2, color="k", zorder=101, label=latexstring(L"$\beta^{\,}_{\mathrm{tri}}\,\ln |\lambda-\lambda^{\,}_{2N,\mathrm{tri}}|+\mathrm{const}$"))

	# 	p, = axins.plot(numpy.log(numpy.abs(lam[30:96].-poptg[2])), numpy.log(numpy.abs(mags[1, 30:96, 1, 2])), linewidth=0.1, marker="x", color=colors[4], markersize=6, markeredgewidth=1.5, zorder=100)	
	# 	# # p, = axins.plot(numpy.log(numpy.abs(lam[10:69].-0.65)), numpy.log(numpy.abs(mags[1, 1, 10:69, 2])), linewidth=0.1, marker="x", color=colors[4], markersize=6, markeredgewidth=1.5, zorder=100)	
	# 	# p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
	# end

	# if N==128
	# 	println(lam[[80:95; 110:120]])
	# 	r_ = lam[[80:95; 110:120]]

	# 	poptf, pcov = scipy.optimize.curve_fit(py"f", collect(r_), numpy.abs(mags[1, [80:95; 110:120], 1, 2]), [1, 0.5, 0.15, 0])
	# 	perrf = numpy.sqrt(numpy.diag(pcov))
	# 	# chi2 = scipy.stats.chisquare(numpy.abs(mags[1:end, t, 1]), py"f"(collect(r_), poptf...), length(r_)-5)
	# 	# poz = chi2[2]
	# 	println(poptf, perrf)
	...
	# 	poptg, pcov = scipy.optimize.curve_fit(py"g", collect(r_), numpy.log(numpy.abs(mags[1, 70:90, 1, 2])), [0.5, 0.51, 1])
	# 	# poptg, pcov = scipy.optimize.curve_fit(py"g", collect(r_), numpy.log(numpy.abs(mags[1, 1, 80:95, 2])), [0.5, 1])
	# 	perrg = numpy.sqrt(numpy.diag(pcov))
	# 	# chi2 = scipy.stats.chisquare(numpy.abs(mags[1:end, t, 1]), py"f"(collect(r_), poptg...), length(r_)-5)
	# 	# poz = chi2[2]
	...
	# 	r2_ = py"r2"(numpy.log(numpy.abs(mags[1, 70:90, 1, 2])), py"g"(collect(r_), poptg...))
	# 	# println(r2_)
	...
	p2, = ax.plot(lam, numpy.abs(stringSPT[1, :, 1, 4]), markeredgewidth=1, lw=1, zorder=105, label=latexstring(L"$|m^{y}_{\mathrm{sta, SPT, 1}}|$ : $2N=%$(N)$"))
