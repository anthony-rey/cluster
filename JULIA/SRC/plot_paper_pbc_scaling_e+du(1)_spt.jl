...
for t=1:length(theta)
	...
	p1, = ax[1].plot(1 .-lam, slopes[:, t, 1], marker="x", markersize=10, color=colors[7], linewidth=0, markeredgewidth=3, zorder=100)
	p1.set_marker(matplotlib.markers.MarkerStyle(p1.get_marker(), capstyle="round"))
	o1, = ax[1].plot(x, py"g"(x, popt...), markersize=0, linewidth=3, color=colors[1], zorder=100, label=latexstring(L"$v^{\,}_{1,0} - %$(round(popt[3]; digits=3)) \sim (1-\lambda)^{%$(round(popt[2]; digits=3)) \pm %$(round(perr[2]; digits=3))}$"))
	...
	p2, = ax[2].plot(1 .-lam, slopes[:, t, 3], marker="x", markersize=10, color=colors[7], linewidth=0, markeredgewidth=3, zorder=100)
	p2.set_marker(matplotlib.markers.MarkerStyle(p2.get_marker(), capstyle="round"))
	o2, = ax[2].plot(x, py"g"(x, popt...), markersize=0, linewidth=3, color=colors[1], zorder=100, label=latexstring(L"$v^{\,}_{3,2} - %$(round(popt[3]; digits=3)) \sim (1-\lambda)^{%$(round(popt[2]; digits=3)) \pm %$(round(perr[2]; digits=3))}$"))
