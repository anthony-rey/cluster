...
plot_lines_correlations_ft = true
...
for folder in folders
	...
	if plot_lines_spins_ft
		...
		for t=1:length(theta)
			...
			ax.set_ylabel(latexstring(L"$f^{z}_{\mathrm{loc}}$"))
	...
	if plot_lines_spins
		...
		for t=1:length(theta)
			...
			fig, ax = plt.subplots(1, 2, figsize=(7.7, 3.5), gridspec_kw=Dict("width_ratios" => [1.8, 1]))
			plt.subplots_adjust(wspace=0.4)

			axins1 = ax[1].inset_axes([0.65, 0.27, 0.3, 0.4])
			...
			ax[1].plot(lam, ft_maxs_spins_z[:, t, 1], linewidth=1, marker=".", color=colors[1], markersize=5, zorder=100, label=latexstring(L"$f^{z}$"))	
			p, = ax[1].plot(lam, numpy.abs(mags[:, t, 1]), linewidth=1, marker="x", color=colors[4], markersize=4, markeredgewidth=1.2, zorder=100, label=latexstring(L"$|m^{z}_{\mathrm{uni}}|$"))	
			p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
			for j=1:length(mags[6:end, t, 1])-1
				ax[2].scatter(ft_maxs_spins_z[5+j, t, 1], numpy.abs(mags[5+j, t, 1]), marker="o", color=cmap(j), s=40, zorder=100)
			end
			# p, = axins1.loglog(lam[6:end].-0.243, numpy.abs(mags[6:end, t, 1]), linewidth=0.7, marker="x", color=colors[4], markersize=6, markeredgewidth=1.5, zorder=100)	
			...
			p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
			ax[1].plot(lam[6:end], 100*lam[6:end], ls="-", lw=2, color=colors[7], zorder=101, label=latexstring(L"fit$^{\,}_{\mathrm{log}}$"))
			...
			cb = fig.colorbar(matplotlib.cm.ScalarMappable(norm=matplotlib.colors.Normalize(vmin=numpy.min(lam[6:end]), vmax=numpy.max(lam[6:end])), cmap=cmap), ax=ax[2], drawedges=false)
			cb.set_label(latexstring(L"$\lambda$"))

			ax[1].annotate("(c)", xy=(0.4, 0.05), xycoords="axes fraction", fontsize=16)
			ax[2].annotate("(d)", xy=(0.4, 0.05), xycoords="axes fraction", fontsize=16)

			ax[1].set_xlabel(latexstring(L"$\lambda$"))
			ax[2].set_xlabel(latexstring(L"$f^{z}$"))
			ax[2].set_ylabel(latexstring(L"$|m^{z}_{\mathrm{uni}}|$"))
			axins1.set_xlabel(latexstring(L"$\ln |\lambda-\lambda^{\,}_{\mathrm{tri}}|$"), fontsize=14)
			axins1.set_ylabel(latexstring(L"$\ln |m^{z}_{\mathrm{uni}}|$"), fontsize=14)

			ax[1].legend(loc="center left", ncol=1, labelspacing=0.8, columnspacing=0.8, handletextpad=0.6, handlelength=1.8, borderaxespad=0, bbox_to_anchor=(0.02, 0.3)).set_zorder(200)

			ax[2].set_xticks([0.3, 0.35, 0.4])
			# ax[2].set_yticks([0.3, 0.4])
			axins1.set_yticks([-1.5, -1])
			axins1.tick_params(labelsize=14)
		
			ax[1].set_ylim([-0.02, 0.41])
			...
			if savePlot
				py"createFolderIfMissing"(graphFolder * folder)
				fig.savefig(graphFolder * folder * "spins_r=$(r_spins)_theta=$(theta[t])" * ".pdf")
				fig.clf()
			end
