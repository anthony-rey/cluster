...
for folder in folders
	...
	for i=1:length(dataFilenames)
		...
		p, = axes[1].plot(range(1, r), corrZ, marker="1", color=colors[7], ls="-", lw=0.5, markersize=5, markeredgewidth=1.5, label=latexstring(L"$C^{z}$"), zorder=100)
		p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
		p, = axes[2].plot(range(1, r), corrZ_conn, marker="1", color=colors[7], ls="-", lw=0.5, markersize=5, markeredgewidth=1.5, label=latexstring(L"$C^{z,\mathrm{c}}$"), zorder=100)
		p.set_marker(matplotlib.markers.MarkerStyle(p.get_marker(), capstyle="round"))
		...
		axes[2].annotate("(c)", xy=(0.945, 0.08), xycoords="axes fraction", fontsize=16)
		...
		axins1.plot(ftfreq[sort], numpy.abs(ft[sort]), marker="x", markersize=2, linewidth=0.5, color=colors[7], zorder=110, label=latexstring(L"FT$^{z}$"))

		ft = numpy.fft.fft(corrZ_conn)
		ftfreq = numpy.fft.fftfreq(length(corrZ_conn))
		sort = numpy.argsort(ftfreq).+1
		axins2.plot(ftfreq[sort], numpy.abs(ft[sort]), marker="x", markersize=2, linewidth=0.5, color=colors[7], zorder=110, label=latexstring(L"FT$^{z,\mathrm{c}}$"))
